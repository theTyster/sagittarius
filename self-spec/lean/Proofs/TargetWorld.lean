...
Lifted verbatim from `thoughts/target-world-shape.lean` (emitted by
`model-obligations`, Stage 3) into the `Proofs/` Lean library so the I-1..I-7
invariant proofs can `import Proofs.TargetWorld`. The shape file itself lives
outside this lib's source tree and is not importable as a module; this is its
in-lib home.
