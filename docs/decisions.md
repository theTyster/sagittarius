...
>
> **Bundle A landed (C-8 / I-9 — F-13, 2026-06-10).** The first two F-12 digest-boundary guards are realized + lock-tested as pure *realization-boundary* mechanics at the loopback chokepoint (+ its byte-identical mirror): a non-stage `targetStage` → `digest_boundary_malformed`, and a forward target (`targetIdx > cursor`) → `forward_loopback`. They kill F-12 mechanisms #1 (phantom `-1`) and #3 (forward-gap loopback) but **NOT** the dominant #2 (a re-spelled-`gapClass` re-mint, which is a *backward* loopback) — that awaits the finite-domain budget re-key (I-3-premise / B1) + the I-3 Lean re-statement (B2). C-8/I-9 are **not yet Lean invariants** (that is B2), so the tables below still index the proven core. See [`../thoughts/FINDINGS.md`](../thoughts/FINDINGS.md) F-13.
...
| D-7 | **`LOOP_LIMIT = 1`** recovery per gap-class-per-stage. | ⚠️ **F-12: the no-oscillation claim is falsified in the realization** — the budget keys on agent-authored freeform strings (fresh spellings mint fresh budgets), so the finiteness premise is unenforced. **F-13: C-8/I-9 guards LANDED (Bundle A)** — phantom-stage and forward-gap loopbacks now hard-stop — but the *dominant* re-spelled-`gapClass` re-mint is a **backward** loopback those guards don't touch; it needs the finite-domain budget re-key (I-3-premise / B1, pending). Tunable upward for kimmy |
